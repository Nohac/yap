...
impl<'a, T, F, S> SepByCtrl<'a, T, F, S> {
    pub(crate) fn new(tokens: &'a mut T, parser: F, separator: S) -> Self {
        Self {
            tokens,
            parser,
...
impl<'a, T, F, S, Output> Tokens for SepByCtrl<'a, T, F, S>
where
    T: Tokens,
    F: FnMut(&mut T) -> ControlFlow<(), Option<Output>>,
    S: FnMut(&mut T) -> bool,
{
    type Item = Output;
    type Location = T::Location;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            match (self.parser)(self.tokens) {
                ControlFlow::Continue(Some(output)) => {
                    self.needs_separator = true;
                    return Some(output);
                }
                ControlFlow::Continue(None) => {
                    self.tokens.next();
                    // Need to see if the next token is a separator without
                    // advancing the itereator
                    let last_good_pos = self.tokens.location();
                    self.needs_separator = (self.separator)(self.tokens);
                    self.tokens.set_location(last_good_pos);
                    continue;
                }
                ControlFlow::Break(()) => {
